...
#define SHOULDER_GEAR_RATIO 50.0      // Shoulder gear ratio multiplier 60:12 * 10:1 = 50:1
#define ELBOW_GEAR_RATIO 50.0         // Elbow gear ratio multiplier 60:12 * 10:1 = 50:1
#define SYRINGE_GEAR_RATIO 8          // Syringe threaded rod 'gear ratio', 1 revolution = 8mm displacement
...
const float shoulderMinDeg = -120.0; // Shoulder rotates clockwise, negative angle changes.
const float shoulderMaxDeg = 0.0; // Upper Shoulder limit, where the limit switch is.

float targetAngleElbow = 0.0;
float dynTargetElbow = 0.0; // dynamic target clamped but the limits.
float currentAngleElbow = 0.0;
float elbowMinDeg = 0.0; // dynamic limit effected by shoulder/Upper arm pose
float elbowMaxDeg = 0.0; // dynamic limit effected by shoulder/Upper arm pose
    ...
    if (!emergencyStop) {
        switch (currentState) {
            ...
            case IVK_PROCESSING:
                ...
                q_2 = acos(fmax(fmin(((pow(Radius - B_L_1 - P_L_4, 2) + pow(Z_pos - O_Z_offset - P_Z_3, 2)) / (2.0 * A * A)) - 1.0, 1.0), -1.0)); //fmax and fmin to ensure q2 doesn't become complex if target position exceeds physical envelope

                // Safety check for invalid math results
                if (isnan(q_2) || isinf(q_2)) {
                    Serial.println(F("Error: Invalid IK solution (target outside workspace)."));
                    currentState = READY;
                    break;  // Exit early to avoid propagating bad values
                }
                ...
                // Safety check for q_1
                if (isnan(q_1) || isinf(q_1)) {
                    Serial.println(F("Error: Invalid IK solution (undefined shoulder angle)."));
                    currentState = READY;
                    break;
                }
                ...
                // Final sefety check before issuing motion commands
                if (isnan(targetAngleWaist) || isnan(targetAngleShoulder) || isnan(targetAngleElbow) || isinf(targetAngleWaist) || isinf(targetAngleShoulder) || isinf(targetAngleElbow)) {
                    Serial.println(F("Error: Computed joint angle NaN/Inf. Movement aborted."));
                    currentState = READY;
                    break;
                }
                ...
                break;
            case MOVING:
                // Update elbow limits dynamically
                updateElbowLimits(); // returns a constrained dynTargetElbow
                ...
                targetStepsElbow = ((-dynTargetElbow * ELBOW_GEAR_RATIO) * (200L * ARM_MICROSTEP_FACTOR)) / 360L; 
...
void processCommand(String cmd) {
    ...
    if (cmd != "") { // have the command not recognised statement at the top of the else if hierachy so it doesn't exit early when checking calibrationFlag
        ...
        if (cmd == "!") {
            ...
            // waist can rotate +- infinity degrees, so no limits
            currentState = MOVING;
            Serial.print(F("New relative waist angle: "));
            ...
            // apply constant shoulder limits
            targetAngleShoulder = constrain(targetAngleShoulder, shoulderMinDeg, shoulderMaxDeg);
            currentState = MOVING;
            Serial.print(F("New relative shoulder angle: "));
            ...
            currentState = MOVING;
            Serial.print(F("New relative elbow angle: "));
...
void updateElbowLimits() {
    // Read current shoulder motor angle (degrees) from steps
    currentAngleShoulder = (shoulder.currentPosition() * 360.0) / (200L * ARM_MICROSTEP_FACTOR * SHOULDER_GEAR_RATIO);

    // Compute theta_1 (radians) consistent with IVK definition
    theta_1 = (currentAngleShoulder * degToRad) + theta_offset_1;

    // Compute theta_2 allowable range (radians), dependent on theta_1
    theta_2_min = (-M_PI + theta_1 + theta_offset_2) + theta_offset_4;
    theta_2_max = (theta_1 + theta_offset_3) + theta_offset_4;

    // Convert limits to degrees
    elbowMinDeg = theta_2_min * radToDeg;
    elbowMaxDeg = theta_2_max * radToDeg;

    // Clamp the dynamic target within the moving limits
    dynTargetElbow = constrain(targetAngleElbow, elbowMinDeg, elbowMaxDeg);
}