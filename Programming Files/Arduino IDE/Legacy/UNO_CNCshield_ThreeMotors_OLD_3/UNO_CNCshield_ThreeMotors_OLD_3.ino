            ...
            case HOMING:
                switch (homingStep) {
                    case HOMING_SHOULDER:
                        if (homeAxis(shoulder, SHOULDER_LIMIT_SWITCH_PIN)) {
                            homingStep = HOMING_ELBOW;
                        }
                        break;
                    case HOMING_ELBOW:
                        if (homeAxis(elbow, ELBOW_LIMIT_SWITCH_PIN)) {
                            homingStep = HOMING_WAIST;
                        }
                        break;
                    case HOMING_WAIST:
                        if (homeAxis(waist, WAIST_LIMIT_SWITCH_PIN)) {
...
bool homeAxis(AccelStepper &motor, int limitSwitchPin) {

    switch (phase) {
        case MOVE_AWAY_1:
            motor.setMaxSpeed(200);       // low speed for safety
            motor.setAcceleration(3000);  // high acceleration for near instantaneous stopping
            motor.stop();
            motor.move(-200); // Move away slightly
            phase = MOVE_TOWARDS_SWITCH;
            break;
        
        case MOVE_TOWARDS_SWITCH:
            if (motor.distanceToGo() == 0) {
            motor.move(50000); // Arbitrary large negative move
        ...
        case MOVE_AWAY_2:
            if (motor.distanceToGo() == 0) {
                ...
                motor.move(50000); // Move slowly back
        ...
        case FINAL_APPROACH:
            ...
            if (digitalRead(limitSwitchPin) == HIGH) {
                ...
                motor.setMaxSpeed(1000);        // Revert to default speed
                motor.setAcceleration(500);     // Revert to default acceleration
