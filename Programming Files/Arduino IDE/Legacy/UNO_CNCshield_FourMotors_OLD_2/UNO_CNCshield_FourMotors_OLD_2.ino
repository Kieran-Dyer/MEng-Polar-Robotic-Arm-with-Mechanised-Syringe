...
#define SYRINGE_DIAMETER 4.75     // Cross sectional area of the syringe in mm
#define SYRINGE_MAXIMUM_DRAW 1000   // maximum capacity in uL (example: 1000 = 1 mL)
...
const float syringeMaxDraw = SYRINGE_MAXIMUM_DRAW / crossSectionArea; // max travel in mm (volume / area = distance)
...
void setup() {
    ...
    syringe.setMaxSpeed(100 * SYRINGE_MICROSTEP_FACTOR/16);
    syringe.setAcceleration(5 * SYRINGE_MICROSTEP_FACTOR/16);
    ...
    if (!emergencyStop) {
        switch (currentState) {
            ...
            case MOVING:
                ...
                if (masterTime <= 0) {
                    Serial.println("No arm movement required (targets already reached).");
                    currentState = READY;
                    break;
                } else {
            ...
            case SYRINGE_MIX:
                ...
                if(mixSyringe()) {
                    Serial.println("Syringe mixing complete");
                    mixPhase = MIX_DRAW;
            ...
            case HELP:
                ...
                Serial.println("home - home the system");
                Serial.println("move motor angle - Move motor by angle");
                Serial.println("point motor angle - Point motor to relative angle");
                Serial.println("Available motors: waist, shoulder, elbow");
                Serial.println("inject syringe volume - inject the sringe by a volume in microlitres");
                Serial.println("draw syringe volume - draw the sringe by a volume in microlitres");
                Serial.println("goto (X, Y, Z) - move the end effector to a cartesian coordinate relative to (0, 0, 0)");
                currentState = READY;
                break;
        }
    }
...
void processCommand(String cmd) {
    ...
    if (cmd == "!") {
        ...
        if(targetPositionSyringe < 0) {
            targetPositionSyringe = 0;
            Serial.println("WARNING: Command clamped to 0 (syringe lower limit).");
        } else if (targetPositionSyringe > syringeMaxDraw) {
            targetPositionSyringe = syringeMaxDraw;
            Serial.println("WARNING: Command clamped to max (syringe upper limit).");
        }

        currentState = SYRINGE;

        Serial.print("Syringe injecting to: ");
        ...
        Serial.print("Syringe drawing to: ");
        Serial.print(volume, 2);
        Serial.print(" uL : ");
        Serial.print(targetPositionSyringe, 2);
        Serial.println(" mm");  
        ...
        if (timePos > 0) {
            ...
            if (volume > SYRINGE_MAXIMUM_DRAW) volume = SYRINGE_MAXIMUM_DRAW;
            ...
            Serial.print("Mixing syringe: ");
            Serial.print(volume, 1);
            Serial.print(" uL * ");
            Serial.print(mixCyclesRequested);
            Serial.print(" cycles (");
            Serial.print(volume / crossSectionArea, 2);
            Serial.println(" mm travel per half-cycle)");
        } else {
            Serial.println("Invalid syntax.  Use: mix syringe <uL> times <n>");
        }
