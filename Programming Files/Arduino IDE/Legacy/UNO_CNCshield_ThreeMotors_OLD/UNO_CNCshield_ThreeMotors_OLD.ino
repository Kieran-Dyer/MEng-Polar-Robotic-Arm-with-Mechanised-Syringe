    ...
    if (!emergencyStop) {
        switch (currentState) {
            ...
            case MOVING:
                ...
                elbow.moveTo((targetAngleElbow * (200L * MICROSTEP_FACTOR)) / 360L);
                waist.run();
                shoulder.run();
                elbow.run();
...
void processCommand(String cmd) {
    ...
    if (cmd == "!") {
        ...
        cmd = cmd.substring(10);
        newTarget = cmd.toFloat();
        targetAngleElbow += newTarget;
        currentState = MOVING;
        Serial.print("New elbow angle: ");
        Serial.println(targetAngleElbow, 2);
    }
    else if (cmd.startsWith("point elbow")) {
        cmd = cmd.substring(11);
        newTarget = cmd.toFloat();
        currentAngleElbow = elbow.currentPosition() * (360.0 / (200L * MICROSTEP_FACTOR));
        currentAngleElbow = fmod(currentAngleElbow, 360);
        if (currentAngleElbow < 0) currentAngleElbow += 360;
        diff = newTarget - currentAngleElbow;
        if (diff > 180) diff -= 360;
        else if (diff < -180) diff += 360;
        targetAngleElbow += diff;
        currentState = MOVING;
