    ...
    if (!emergencyStop) {
        switch (currentState) {
            ...
            case HELP:
                ...
                Serial.println(F("inject syringe volume - inject the sringe by a volume in microlitres"));
                Serial.println(F("draw syringe volume - draw the sringe by a volume in microlitres"));
...
void processCommand(String cmd) {
    ...
    if (!calibrationFlag) { // Allows movement when not in callibration mode
        ...
        if (cmd == "!") {
            ...
            if (timePos > 0) {
                ...
                Serial.println(F("Invalid syntax.  Use: mix syringe <uL> times <n>"));
            }
        }
