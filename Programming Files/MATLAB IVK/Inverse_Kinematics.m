...
% User Calibration offsets (mm)
Cal_X = 0; % +X tool tip correction
Cal_Y = 0; % +Y correction
Cal_Z = 0; % +Z correction
...
X_pos = X_pos + Cal_X; % in mm
Y_pos = Y_pos + Cal_Y; % in mm
Z_pos = Z_pos + Cal_Z; % in mm

% ===== Mathematical model Offsets (in mm) =====
% Orange Vector
O_X_offset = -117.316;
O_Y_offset = -88.741;
O_Z_offset = 45.064;
...
q_2 = acos(max(min(((Radius - B_L_1 - P_L_4)^2 + (Z_pos - O_Z_offset - P_Z_3)^2) / (2 * A * A) - 1, 1), -1));
